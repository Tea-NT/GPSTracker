...
static void Decoder_amr_reset( Decoder_amrState *state, enum Mode mode )
{
   ...
   GM_memset(state->Cb_gain_averState->cbGainHistory, 0, sizeof( Word32 )*L_CBGAINHIST);
   ...
   GM_memset( state->old_exc, 0, sizeof( Word32 )*( PIT_MAX + L_INTERPOL ) );

   if ( mode != MRDTX )
      GM_memset( state->mem_syn, 0, sizeof( Word32 )*M );
   ...
   if (mode != MRDTX)
   {
   	GM_memset( state->excEnergyHist, 0, sizeof( Word32 )*9 );
   }
   GM_memset( state->ltpGainHistory, 0, sizeof( Word32 )*9 );
   ...
   GM_memset( state->lsfState->past_r_q, 0, sizeof( Word32 )*M );
   ...
   /* Static vectors to zero */
   GM_memset( state->background_state->frameEnergyHist, 0, sizeof( Word32 )*L_ENERGYHIST );
   /* Initialize hangover handling */
   state->background_state->bgHangover = 0;
   /* phDispReset */
   GM_memset( state->ph_disp_st->gainMem, 0, sizeof( Word32 )*PHDGAINMEMSIZE );
   ...
   if ( mode != MRDTX ) 
   {
      ...
      {
         GM_memcpy( &state->dtxDecoderState->lsf_hist[M * i], &state->dtxDecoderState->lsf_hist[0], sizeof( Word32 )*M );
      }
      GM_memset( state->dtxDecoderState->lsf_hist_mean, 0, sizeof( Word32 ) * M * DTX_HIST_SIZE );
      ...
      indice, Word32 *lsp1_q )
{
   ...
   else 
   {
      ...
      if ( mode != MRDTX ) 
      {
         ...
         GM_memcpy( st->past_r_q, lsf1_r, sizeof( Word32 )*M );
      }
      else
      {
         for ( i = 0; i < M; i++ ) 
   ...
   GM_memcpy( st->past_lsf_q, lsf1_q, sizeof( Word32 )*M );
...
static void A_Refl( Word32 a[], Word32 refl[] )
{
   ...
   /* initialize states */
   GM_memcpy( aState, a, sizeof( Word32 )*M );
   ...
   for ( i = M - 1; i >= 0; i-- ) 
   {
      ...
      GM_memcpy( aState, bState, sizeof( Word32 )*i );
   }
   return;
ExitRefl:
   GM_memset( refl, 0, sizeof( Word32 )*M );
}
...
static void Build_CN_code( Word32 *seed, Word32 cod[] )
{
   ...
   GM_memset( cod, 0, sizeof( Word32 )*L_SUBFR );
      ...
      , Word32 update )
{
   ...
   /* Copy mem[] to yy[] */
   GM_memcpy( tmp, mem, sizeof( Word32 )*M );
   yy = tmp + M;
   yy_limit = yy + lg;
   a0 = a[0];
   ...
   GM_memcpy( y, &tmp[M], sizeof( Word32 )*lg );

   /* Update of memory if update==1 */
   if ( update ) {
      GM_memcpy( mem, &y[lg - M], sizeof( Word32 )*M );
   }
   return overflow;
}
      ...
      Word32 A_t[] )
{
   ...
   if ( ( st->dtxHangoverAdded != 0 ) & ( st->sid_frame != 0 ) ) 
   {
      ...
      GM_memcpy( &st->lsf_hist[ptr], &st->lsf_hist[st->lsf_hist_ptr], sizeof( Word32 )*M );
       ...
       */
      st->log_en = 0;
      GM_memset( lsf, 0, sizeof( Word32 )*M );
      ...
      GM_memcpy( st->lsf_hist_mean, st->lsf_hist, sizeof( Word32 )*80 );
   ...
   if ( st->sid_frame != 0 ) {
       ...
       */
      GM_memcpy( st->lsp_old, st->lsp, sizeof( Word32 )*M );
       ...
       */
         ...
         GM_memcpy( lsfState->past_r_q, &past_rq_init[parm[0] * M], sizeof( Word32 )*M );
         D_plsf_3( lsfState, MRDTX, 0, &parm[1], st->lsp );

         /* reset for next speech frame */
         GM_memset( lsfState->past_r_q, 0, sizeof( Word32 )*M );
         log_en_index = parm[4];
   ...
   GM_memcpy( lsf_int_variab, lsf_int, sizeof( Word32 )*M );
   ...
   GM_memcpy( lsfState->past_lsf_q, lsf_int, sizeof( Word32 )*M );
   ...
   GM_memcpy( &A_t[0], acoeff, sizeof( Word32 )*MP1 );
   GM_memcpy( &A_t[MP1], acoeff, sizeof( Word32 )*MP1 );
   GM_memcpy( &A_t[MP1 <<1], acoeff, sizeof( Word32 )*MP1 );
   GM_memcpy( &A_t[MP1 + MP1 + MP1], acoeff, sizeof( Word32 )*MP1 );
      ...
      , Word32 *lsp2_q )
{
   ...
   if ( bfi != 0 ) 
   {
      ...
      GM_memcpy( lsf2_q, lsf1_q, sizeof( Word32 )*M );
   ...
   GM_memcpy( st->past_lsf_q, lsf2_q, sizeof( Word32 )*M );
...
static void decode_8i40_31bits( Word16 index[], Word32 cod[] )
{
   ...
   GM_memset( cod, 0, sizeof( Word32 )*L_CODE );
      ...
      lsf_out[] )
{
   ...
   switch ( i_subfr ) {
      ...
      case 120:
         GM_memcpy( lsf_out, lsf_new, sizeof( Word32 )*M );
         break;
   }
}
                    ...
                    Word32 pitch_fac, Word32 tmp_shift)
{
   ...
   if ( ( mode != MR122 ) & ( mode != MR102 ) & ( mode != MR74 ) & ( impNr < 2 )
      ...
      GM_memcpy( inno_sav, inno, sizeof( Word32 )*L_SUBFR );
      GM_memset( inno, 0, sizeof( Word32 )*L_SUBFR );
      ...
      frame[] )
{
   ...
   GM_memcpy( &st->lsf_hist[st->lsf_hist_ptr], lsf, sizeof( Word32 )*M );
      ...
      enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   ...
   GM_memcpy( prev_lsf, st->lsfState->past_lsf_q, sizeof( Word32 )*M );
   ...
   GM_memcpy( st->lsp_old, lsp_new, sizeof( Word32 )*M );
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      /* MR122 */
         ...
         */
      GM_memcpy( exc_enhanced, st->exc, sizeof( Word32 )*L_SUBFR );
      ...
      if ( overflow ) {
         ...
         GM_memcpy( st->mem_syn, &synth[i_subfr + 30], sizeof( Word32 )*M );
      }
         ...
         */
      GM_memcpy( &st->old_exc[0], &st->old_exc[L_SUBFR], sizeof( Word32 )*( PIT_MAX + L_INTERPOL ) );
      ...
      Word32 *Az_4 )
{
    ...
    * Post filtering
    */
   GM_memcpy( syn_work, syn, sizeof( Word32 )*L_FRAME );
   Az = Az_4;
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      GM_memcpy( h, Ap3, sizeof( Word32 )*MP1 );
      GM_memset( &h[M +1], 0, sizeof( Word32 )*( 22 - M - 1 ) );
      ...
      *p1 = *p1 - ( ( temp2 * st->preemph_state_mem_pre ) >> 15 );
      ...
      if (overflow){
         ...
         GM_memcpy(st->mem_syn_pst, &syn[i_subfr + 30], sizeof( Word32 )*M);
      }
   ...
   GM_memcpy( &syn_work[- M], &syn_work[L_FRAME - M], sizeof( Word32 )*M );
   return;
}
...
static void Decoder_amr_exit( Decoder_amrState **state )
{
   if ( state == NULL || *state == NULL )
      return;
   GM_MemoryFree( ( *state )->lsfState );
   GM_MemoryFree( ( *state )->ec_gain_p_st );
   GM_MemoryFree( ( *state )->ec_gain_c_st );
   GM_MemoryFree( ( *state )->pred_state );
   GM_MemoryFree( ( *state )->background_state );
   GM_MemoryFree( ( *state )->ph_disp_st );
   GM_MemoryFree( ( *state )->Cb_gain_averState );
   GM_MemoryFree( ( *state )->lsp_avg_st );
   GM_MemoryFree( ( *state )->dtxDecoderState );
...
static int Decoder_amr_init( Decoder_amrState **state )
{
   ...
   *state = NULL;
   ...
   /* allocate memory */
   if ( ( s = ( Decoder_amrState * ) GM_MemoryAlloc( sizeof( Decoder_amrState ) ) ) ==
         NULL ) {
   ...
   /* DPlsf_init */
   /* allocate memory */
   if ( ( s->lsfState = ( D_plsfState * ) GM_MemoryAlloc( sizeof( D_plsfState ) ) ) ==
         NULL ) {
   ...
   /* ecGainPitchInit */
   /* allocate memory */
   if ( ( s->ec_gain_p_st = ( ec_gain_pitchState * ) GM_MemoryAlloc( sizeof(
   ...
   /* ecGainCodeInit */
   /* allocate memory */
   if ( ( s->ec_gain_c_st = ( ec_gain_codeState * ) GM_MemoryAlloc( sizeof(
   ...
   /* gcPredInit */
   /* allocate memory */
   if ( ( s->pred_state = ( gc_predState * ) GM_MemoryAlloc( sizeof( gc_predState ) ) )
         == NULL ) {
   ...
   /* Cb_gain_averageInit */
   /* allocate memory */
   if ( ( s->Cb_gain_averState = ( Cb_gain_averageState * ) GM_MemoryAlloc( sizeof(
         Cb_gain_averageState ) ) ) == NULL ) {
      //fprintf( stderr, "Cb_gain_averageInit: can not malloc state structure\n" )
      ;
      return-1;
   }
   GM_memset( s->Cb_gain_averState->cbGainHistory, 0, sizeof( Word32 )*L_CBGAINHIST );
   ...
   /* lsp_avgInit */
   /* allocate memory */
   if ( ( s->lsp_avg_st = ( lsp_avgState * ) GM_MemoryAlloc( sizeof( lsp_avgState ) ) )
         == NULL ) {
   ...
   /* Bgn_scdInit */
   /* allocate memory */
   if ( ( s->background_state = ( Bgn_scdState * ) GM_MemoryAlloc( sizeof( Bgn_scdState
         ) ) ) == NULL ) {
   ...
   /* phDispInit */
   /* allocate memory */
   if ( ( s->ph_disp_st = ( ph_dispState * ) GM_MemoryAlloc( sizeof( ph_dispState ) ) )
         == NULL ) {
   ...
   /* dtxDecInit */
   /* allocate memory */
   if ( ( s->dtxDecoderState = ( dtx_decState * ) GM_MemoryAlloc( sizeof( dtx_decState )
         ) ) == NULL ) {
...
static int Post_Filter_reset( Post_FilterState *state )
{
   ...
   GM_memset( state->mem_syn_pst, 0, sizeof( Word32 )*M );
   GM_memset( state->res2, 0, sizeof( Word32 )*L_SUBFR );
   GM_memset( state->synth_buf, 0, sizeof( Word32 )*( L_FRAME + M ) );
   return 0;
}
...
static int Post_Filter_init( Post_FilterState **state )
{
   ...
   *state = NULL;
   ...
   /* allocate memory */
   if ( ( s = ( Post_FilterState * ) GM_MemoryAlloc( sizeof( Post_FilterState ) ) ) ==
         NULL ) {
   ...
   /* allocate memory */
   if ( ( s->agc_state = ( agcState * ) GM_MemoryAlloc( sizeof( agcState ) ) ) == NULL )
   {
...
static int Post_Process_init( Post_ProcessState **state )
{
   ...
   *state = NULL;
   ...
   /* allocate memory */
   if ( ( s = ( Post_ProcessState * ) GM_MemoryAlloc( sizeof( Post_ProcessState ) ) ) ==
         NULL ) {
...
void Speech_Decode_Frame_exit( void **st )
{
   ...
   /* deallocate memory */
   GM_MemoryFree( (( Speech_Decode_FrameState * )st) );
   return;
}
...
void * Speech_Decode_Frame_init(void)
{
   ...
   /* allocate memory */
   if ( ( s = ( Speech_Decode_FrameState * ) GM_MemoryAlloc( sizeof(
